...
def backtest_strategy(data, index_column, bond_etf_column, dividend_index_column, dynamic_trend, hs300_trend, initial_cash=10000, transaction_fee=0.0003):
    """
    ...
    for i in range(len(data)):
        ...
        if i in last_days:
            ...
            cash += portfolio_value * dividend_rate  # 增加股息收益
...
if data is not None:
    ...
    dividend_index_column = '点位'  # 红利指数列
    ...
    hs300_trend = calculate_trend(data, dividend_index_column, window=HS300_TREND_WINDOW)

    final_value, total_return, daily_portfolio, sharpe_ratio, max_drawdown = backtest_strategy(
        data, index_column, bond_etf_column, dividend_index_column, dynamic_trend, hs300_trend
    )
    ...
    # 右轴：红利指数
    ax2 = ax1.twinx()
    ax2.plot(data[x_column], data[dividend_index_column], label='红利指数', color='green')
    ax2.set_ylabel('红利指数', color='green')
